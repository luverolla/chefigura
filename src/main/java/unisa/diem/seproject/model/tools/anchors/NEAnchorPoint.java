...
public class NEAnchorPoint implements AnchorPoint {
    ...
    public NEAnchorPoint(SelectionTool selectionTool, Canvas canvas) {
        this.canvas = canvas;
    ...
    public void mouseMove(double mouseX, double mouseY) {
        if (sm.selectedShapeProperty.get() != null) {
            if (sm.selectedShapeProperty.get().getBounds().mouseOnNEAnchorPoint(mouseX, mouseY)) {
    ...
    public void mouseDragStart(double mouseX, double mouseY) {
        if (sm.selectedShapeProperty.get() != null && sm.selectedShapeProperty.get().getBounds().mouseOnNEAnchorPoint(mouseX, mouseY)) {
            isDragging = true;
            this.oldX = sm.selectedShapeProperty.get().getBounds().getAnchorPoints()[1].getX();
    ...
    public void mouseDragInProgress(double mouseX, double mouseY) {
        if (sm.selectedShapeProperty.get() != null && isDragging) {
            double deltaX = mouseX - shadow.getBounds().getAnchorPoints()[1].getX();
            if (deltaX > oldWidth / 2 - 5) {
                return;
            }
